...
pub const K_INITIAL_RTT_NS: u64 = 100_000_000; // 100ms (conservative default)
...
test "rtt: min_rtt never increases after lower sample" {
    const testing = std.testing;
    var rtt = RttEstimator{};
    // min_rtt starts at K_INITIAL_RTT_NS = 100_000_000
    rtt.update(150_000_000, 0, 25_000_000); // sample > initial
    try testing.expectEqual(@as(u64, 100_000_000), rtt.min_rtt); // min(initial, sample)
    rtt.update(50_000_000, 0, 25_000_000); // lower sample
    try testing.expectEqual(@as(u64, 50_000_000), rtt.min_rtt);
